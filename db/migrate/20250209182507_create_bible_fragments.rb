class CreateBibleFragments < ActiveRecord::Migration[8.0]
  def change
    create_table :bible_fragments do |t|
      t.references :translation, null: false, foreign_key: { on_delete: :restrict, to_table: :bible_translations }
      t.references :book, null: false, foreign_key: { on_delete: :restrict, to_table: :bible_books }
      t.references :chapter, null: false, foreign_key: { on_delete: :restrict, to_table: :bible_chapters }
      t.references :heading, null: false, foreign_key: { on_delete: :restrict, to_table: :bible_headings }
      t.references :verse, null: true, foreign_key: { on_delete: :restrict, to_table: :bible_verses }
      t.references :segment, null: false, foreign_key: { on_delete: :restrict, to_table: :bible_segments }
      t.boolean :show_verse, null: false
      t.string :kind, null: false
      t.text :content, null: false
      t.integer :position, null: false
      t.belongs_to :fragmentable, polymorphic: true, null: true

      t.timestamps
    end
  end
end
